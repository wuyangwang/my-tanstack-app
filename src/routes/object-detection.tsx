...
function ObjectDetection() {
	...
	const drawDetections = useCallback(() => {
		...
		const targetElement =
			activeTab === "camera" ? videoRef.current : staticImageRef.current;
	...
	return (
		<div className="container mx-auto max-w-5xl space-y-8 px-4 py-10">
			...
			>
				<div className="flex justify-center mb-6">
					<TabsList className="grid w-full max-w-md grid-cols-2">
						<TabsTrigger value="camera">
							<Camera className="mr-2 h-4 w-4" /> 实时摄像头
						</TabsTrigger>
						<TabsTrigger value="image">
							<ImageIcon className="mr-2 h-4 w-4" /> 图片识别
						</TabsTrigger>
					</TabsList>
				</div>
				...
				<div className="grid gap-6 lg:grid-cols-4">
					<Card className="lg:col-span-3 overflow-hidden bg-black relative aspect-video flex items-center justify-center border-2 border-border shadow-2xl rounded-xl">
						<TabsContent
							value="camera"
							className="mt-0 w-full h-full flex items-center justify-center"
						>
							{!isStreaming && (
							...
							<CardContent className="space-y-6">
								<div className="space-y-2">
									...
									<Select value={model} onValueChange={setModel}>
										...
										<SelectContent>
											<SelectItem value="onnx-community/rfdetr_medium-ONNX">
												RF-DETR
											</SelectItem>
											<SelectItem value="Xenova/detr-resnet-50">
												DETR ResNet-50
											</SelectItem>
										</SelectContent>
									</Select>
								...
								{activeTab === "camera" ? (
									...
									<div className="flex flex-col gap-2">
										<Button
											className="w-full font-bold"
											onClick={() =>
												selectedImage && processStaticImage(selectedImage)
											}
								...
								<div className="space-y-2 max-h-[300px] overflow-y-auto pr-2">
									{results.length === 0 ? (
										...
										results.map((res, i) => (
											<div
												key={i}
												className="flex justify-between items-center bg-muted p-2 rounded text-xs border-l-4 border-primary"
											>
			...
			<div className="text-center text-sm text-muted-foreground">
				提示：RF-DETR 模型擅长处理复杂场景，而 DETR ResNet-50
				提供了另一种选择。您可以上传一张图片来测试模型的精准度。
			</div>
		</div>
	);
}
