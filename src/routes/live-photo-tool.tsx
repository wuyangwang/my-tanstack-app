							...
							{loading ? (
								...
								<>
									...
									<div className="relative group">
										...
										<Button
											size="lg"
											className="rounded-full px-10 h-14 text-lg font-bold shadow-lg transition-transform group-hover:scale-105 active:scale-95"
										>
						...
						<div className="relative aspect-video bg-black flex items-center justify-center group overflow-hidden">
							...
							{result.videoUrl && (
								...
								>
									...
									>
										<Disc
											className={`h-5 w-5 ${isHovering ? "animate-spin" : ""}`}
								...
								<div className="flex gap-3">
									...
									{result.videoBlob && (
										<Button
											...
											className="rounded-full font-bold shadow-xl backdrop-blur-md bg-white/20 hover:bg-white/30 text-white border-none"
											onClick={() =>
												downloadFile(result.videoBlob, "live-photo.mp4")
											}
										>
			...
			<div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
				<Card className="bg-muted/30 border-none shadow-sm">
					...
					<CardContent className="text-xs text-muted-foreground leading-relaxed">
						Apple Live Photo 在拍摄瞬间记录了前后 1.5 秒的影音。 在 HEIF
						容器中，它通常包含一个高质量图像轨道和一个 HEVC 短视频轨道。
					</CardContent>
				</Card>
				...
				<Card className="bg-muted/30 border-none shadow-sm">
					...
					<CardContent className="text-xs text-muted-foreground leading-relaxed">
						您可以单独提取高清静态图片（PNG）或原始视频片段（MP4），方便在非
						Apple 设备或社交平台上使用。
					</CardContent>
				</Card>
			</div>
