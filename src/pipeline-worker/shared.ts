...
export async function getPipeline(
	...
	const isMoonshineAsrModel =
		...
		model.toLowerCase().includes("moonshine");
	const autoDevice =
		isWebGPUSupported && !isMoonshineAsrModel ? "webgpu" : "wasm";
	...
	const createPipeline = async (
		targetDevice: InferenceDevice,
		targetDtype: InferenceDtype,
	) => {
	...
	try {
		...
		const canRetryWithQ8 =
			device === "webgpu" && loadedDtype === "fp16" && shouldRetryWithQ8(error);
